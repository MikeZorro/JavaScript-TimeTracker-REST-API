...
let titleBar = document.querySelector("[name=title]")
let descriptionBar = document.querySelector("[name=description]")
addTaskButton.addEventListener("click", function (event) {
    event.preventDefault();
    apiCreateTask(titleBar.value, descriptionBar.value)
    ...
    if (status == 'open') {
        ...
        form.addEventListener("submit", function (event) {
            ...
            apiCreateOperationForTask(taskId, descriptionInput.value).then(
                function (response) {
                    renderOperation(ul, status, taskId, descriptionInput.value, 0)
                }
            )
        })
